# Projet MAOA : Pick-and-Deliver Traveling Salesman Problem (PD-TSP)

Projet réalisé dans le cadre du module **MAOA**  Master AI2D / M2 Androide, Sorbonne Université (2025-2026).

**Auteurs :** Mohamed NAJEM, Jules MAZLUM
...
| Fichier | Description |
|---|---|
| `MAZLUM_NAJEM.ipynb` | Notebook Jupyter contenant l'ensemble du code : modélisation, heuristiques, recherche locale (Swap/2-Opt/VND), formulation PLNE Gurobi, visualisations et expérimentations. |
| `MAZLUM_NAJEM.html` | Export HTML du notebook (lecture rapide sans avoir à exécuter le code). |
...
Formulation compacte de la variante linéaire avec :
- variables de routage `xij`,
- variables de collecte `yik`,
- variables de flot continues `fij` (pour éviter la non-linéarité du produit poids × arc),
- contraintes d'élimination de sous-tours (MTZ),
- contraintes de conservation du flot représentant l'évolution du poids `Wi`.
...
Le détail complet des expérimentations, tableaux de résultats et visualisations (tournées, évolution du poids, comparaisons statistiques) est disponible dans le [rapport complet](./Rapport-MAZLUM_NAJEM.pdf).

## Utilisation

1. Ouvrir `MAZLUM_NAJEM.ipynb` dans Jupyter (ou consulter directement `MAZLUM_NAJEM.html` pour une lecture rapide).
2. Installer les dépendances nécessaires (notamment `gurobipy` pour la partie PLNE  une licence Gurobi est requise).
